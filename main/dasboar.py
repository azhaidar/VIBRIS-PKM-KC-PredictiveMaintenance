...
ESP32_USB_HINTS = ["CH340", "CH343", "CP210", "USB-SERIAL", "USB SERIAL", "FTDI", "SILICON LABS", "COM3"]
...
class GradientGauge(QWidget):
    def __init__(self, title, min_val, max_val, t_warn, t_danger, unit=""):
        ...
        self.lbl_title.setStyleSheet(f"font-size: 13px; font-weight: bold; color: {COL_TEXT_LIGHT};")
        ...
        self.lbl_status.setStyleSheet(f"font-size: 12px; color: {COL_TEXT_DIM};")
...
class Dashboard(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("HMI | VIBRIS PIMNAS")
        self.setWindowFlags(
        ...
        # ===== STATE SESI CHECK 1-MENIT (tombol SESI(K)) =====
        # check_session_active: True selama nunggu firmware kirim hasil
        # (paket JSON type=session_summary) setelah tombol K ditekan.
        # check_session_start_time: dipakai buat hitung mundur di layar,
        # PENANDA SELESAI YANG SEBENARNYA tetap paket session_summary dari
        # firmware, bukan timer ini -- timer cuma buat tampilan progres.
        self.check_session_active = False
        self.check_session_start_time = None
        self.CHECK_SESSION_DURATION_S = 60  # sinkron dengan CHECK_SESSION_DURATION_MS di config.h firmware

        # ===== STATE KALIBRASI 3 MENIT (tombol KALIBRASI) =====
        # Sama pola-nya kayak state Check di atas. Bedanya, tanda "selesai"
        # buat kalibrasi bukan dari paket JSON khusus, tapi dari field
        # "status" yang berubah dari "calibrating" ke status lain (Normal/
        # Waspada/Bahaya/Diam) -- itu tandanya main.ino sudah lewat blok
        # initializeBaselineLearner()+saveBaselineToFlash(), jadi baseline
        # baru PASTI sudah tersimpan ke flash ESP32 (lihat _update_gui).
        self.calibration_active = False
        self.calibration_start_time = None
        self.CALIBRATION_DURATION_S = 180  # sinkron dengan CALIBRATION_DURATION_MS di main.ino firmware
        ...
        # =========================================================================
        # HEADER FULL FEATURE & DIRECT ACTION (TANPA KONFIRMASI)
        # =========================================================================
        header = QHBoxLayout()
        header.setSpacing(1)
        ...
        # Seluruh tombol aksi langsung eksekusi tanpa pop-up dialog konfirmasi
        self.btn_cek1m = QPushButton("SESI(K)")
        self.btn_cek1m.setStyleSheet(_header_btn_style(COL_OK, COL_OK))
        # GANTI: sebelumnya langsung self._send_command('K') -- sekarang
        # lewat _start_check_session() supaya ada state hitung mundur +
        # tombol otomatis nonaktif selama 1 menit (gak bisa dobel pencet),
        # dan hasilnya nanti otomatis muncul sebagai popup kesimpulan.
        self.btn_cek1m.clicked.connect(self._start_check_session)
        ...
        # GANTI: sebelumnya langsung self._send_command('R') -- sekarang
        # lewat _start_calibration() supaya ada hitung mundur 180 detik +
        # tombol nonaktif selama proses, dan begitu selesai otomatis
        # muncul konfirmasi "tersimpan permanen".
        self.btn_recal.clicked.connect(self._start_calibration)
        ...
        # =========================================================================
        # STACKED WIDGET (PENGEMBALIAN STRUKTUR KODE LENGKAP ~1600+ BARIS)
        # =========================================================================
    ...
    def _page_summary(self):
        ...
        health_box = QFrame()
        health_box.setStyleSheet(f"background-color: {COL_PANEL_DARK}; border: 1px solid {COL_TEXT_DIM}; border-radius: 3px;")
        health_box_lay = QVBoxLayout(health_box)
        health_box_lay.setContentsMargins(2, 2, 2, 2)
        health_box_lay.setSpacing(0)

        self.lbl_health_emoji = QLabel("🙂")
        self.lbl_health_emoji.setAlignment(Qt.AlignCenter)
        self.lbl_health_emoji.setStyleSheet("font-size: 40px;")
        health_box_lay.addWidget(self.lbl_health_emoji)

        self.lbl_health_percent = QLabel("--")
        self.lbl_health_percent.setAlignment(Qt.AlignCenter)
        self.lbl_health_percent.setStyleSheet(f"font-size: 16px; font-weight: bold; color: {COL_TEXT_LIGHT};")
        health_box_lay.addWidget(self.lbl_health_percent)

        grid.addWidget(health_box, 1, 1)
    ...
    def _page_machine_select(self):
        ...
        self.lbl_machine_hint = QLabel("Pemilihan mesin akan otomatis mengirim command klaster bearing ke ESP32.")
        self.lbl_machine_hint.setStyleSheet(f"font-size: 6px; color: {COL_TEXT_DIM};")
    ...
    def _read_serial_worker(self):
        while True:
            try:
                ...
                # PENTING: firmware kirim DUA jenis paket JSON lewat baris
                # serial yang sama -- paket telemetri biasa (rms_v, d2, dst,
                # dikirim tiap ~100ms) DAN paket kesimpulan sesi Check
                # (type="session_summary", dikirim SEKALI tiap sesi K
                # selesai, lihat Transmitter_SendSessionSummary() di
                # RaspberryPiDataTransmitter.cpp). Paket kedua ini TIDAK
                # punya field rms_v/d2/status -- kalau dibiarkan lanjut ke
                # bawah, semua field itu bakal kebaca 0.0 dan nyelip jadi
                # titik data palsu di grafik + ikut kerekam ke CSV. Jadi
                # begitu ketauan ini paket kesimpulan, tangani terpisah
                # lewat _handle_session_summary() lalu lewati sisa parsing
                # telemetri di bawah (continue balik ke awal while).
                if data.get("type") == "session_summary":
                    self._handle_session_summary(data)
                    continue
                ...
                # FIX: firmware kirim nama field "rms_x"/"rms_y"/"rms_z"
                # (lihat RaspberryPiDataTransmitter.cpp), BUKAN "vib_x" dst.
                # Sebelum ini dibenerin, ketiga baris di bawah SELALU baca
                # default 0.0 -- makanya breakdown X/Y/Z di layar RAW READING
                # gak pernah kelihatan gerak walau grafik utama (rms_v) jalan
                # normal.
                ...
                # FIX: nama field asli di firmware itu "ml_conf" (bukan
                # "ml_confidence"), "diagnosis" (bukan "diagnosis_label"),
                # "diag_conf" (bukan "diagnosis_confidence"), dan "diag_flags"
                # (bukan "diagnosis_flags"). Sebelum dibenerin, empat baris
                # ini SELALU jatuh ke nilai default-nya masing-masing --
                # confidence TinyML selalu kebaca 0%, label diagnosa selalu
                # "N/A", dan kolom Multi-Fault selalu "Aman" walau motor lagi
                # kena Unbalance/BPFO sekalipun.
                ...
                self.current_diagnosis_flags = self._decode_diagnosis_flags(data.get("diag_flags", 0))

                # BELUM DIKIRIM FIRMWARE (bukan salah nama key -- field ini
                # memang belum ada sama sekali di JSON yang dikirim
                # Transmitter_SendResult()). Dibiarkan pakai nilai default
                # supaya dashboard gak error, TAPI ini artinya kurtosis dan
                # grafik "Frequency Spectrum (FFT)" di halaman PROCESSED
                # gak akan pernah keisi data asli sampai firmware-nya
                # ditambah pengiriman field ini -- ini keputusan fitur yang
                # perlu didiskusikan dulu (kurtosis: field-nya sudah ada di
                # SensorFeatures, tinggal ditambah ke JSON; fft_hz/fft_mag:
                # ngirim seluruh array FFT lewat serial JSON tiap siklus itu
                # berat/lambat, perlu dipikirkan caranya dulu sebelum asal
                # ditambah).
    ...
    def _decode_diagnosis_flags(self, flags_int):
        """Terjemahkan bitmask diag_flags (angka 0-15 dari firmware) jadi
        teks yang bisa dibaca. Pemetaan bit HARUS sama persis dengan
        komentar di SharedTypes.h: bit0=unbalance, bit1=misalignment,
        bit2=BPFO, bit3=BPFI. Kalau firmware nanti nambah bit baru, tinggal
        tambah satu baris di list BIT_LABELS ini, gak perlu ubah tempat
        lain."""
        try:
            flags_int = int(flags_int)
        ...
        active = [label for bit, label in BIT_LABELS if flags_int & (1 << bit)]
        return " + ".join(active) if active else "Aman"

    def _start_check_session(self):
        """Dipanggil pas tombol SESI(K) ditekan. Ngirim command 'K' ke
        firmware (mulai jendela hitung 60 detik di ESP32 -- lihat
        startCheckSession() di CheckSession.cpp), lalu nyalain state lokal
        buat nampilin hitung mundur di tombolnya sendiri.

        PENTING: yang nentuin sesi ini SELESAI itu paket JSON
        type=session_summary yang beneran dikirim firmware (ditangani di
        _handle_session_summary), BUKAN timer 60 detik di sini -- timer di
        sini cuma buat kasih tau user progresnya, kalau paketnya gak pernah
        nyampe (misal kabel serial kecabut), ada batas aman di _update_gui
        yang otomatis balikin tombol ke kondisi normal supaya gak nyangkut
        selamanya."""
        ...
        if self.check_session_active:
            return  # sesi masih jalan, jangan kirim K dobel
        if self.calibration_active:
            QMessageBox.warning(self, "Perhatian", "Tunggu kalibrasi selesai dulu sebelum mulai sesi Check.")
            return
        self._send_command('K')
        self.check_session_active = True
        self.check_session_start_time = time.time()
        self.btn_cek1m.setEnabled(False)
        self.btn_cek1m.setText(f"CEK...{self.CHECK_SESSION_DURATION_S}s")

    def _start_calibration(self):
        """Dipanggil pas tombol KALIBRASI ditekan. Ngirim command 'R' ke
        firmware (mulai ulang jendela kalibrasi 180 detik -- lihat
        startCalibrationPhase() di InitialBaselineCalibrator.cpp), lalu
        nyalain state lokal buat hitung mundur di tombolnya sendiri.

        PENTING soal "kapan dianggap selesai": beda dengan sesi Check yang
        ditandai paket JSON khusus, kalibrasi ditandai dari field "status"
        yang berubah DARI "calibrating" KE status lain (lihat _update_gui).
        Begitu berubah, itu artinya main.ino sudah lewat baris
        initializeBaselineLearner()+saveBaselineToFlash() -- baseline baru
        SUDAH PASTI tersimpan ke flash ESP32 saat itu juga."""
        if self.selected_machine_idx < 0:
            QMessageBox.warning(self, "Perhatian", "Silakan pilih Target Mesin terlebih dahulu!")
            return
        if self.calibration_active:
            return  # kalibrasi masih jalan, jangan kirim R dobel
        if self.check_session_active:
            QMessageBox.warning(self, "Perhatian", "Tunggu sesi Check (1 menit) selesai dulu sebelum kalibrasi ulang.")
            return
        self._send_command('R')
        self.calibration_active = True
        self.calibration_start_time = time.time()
        self.btn_recal.setEnabled(False)
        self.btn_recal.setText(f"KALIBRASI...{self.CALIBRATION_DURATION_S}s")

    def _show_calibration_done_dialog(self):
        """Konfirmasi ke user bahwa baseline baru sudah tersimpan PERMANEN
        (flash/NVS ESP32, lihat saveBaselineToFlash() di
        InitialBaselineCalibrator.cpp) -- tetap ada walau device di-restart
        atau kabel dicabut, sampai dihapus manual (tombol BASE(Z)) atau
        ditimpa kalibrasi baru (tombol KALIBRASI lagi)."""
        box = QMessageBox(self)
        box.setWindowTitle("Kalibrasi Selesai")
        box.setIcon(QMessageBox.Information)
        box.setText(
            "✅ Kalibrasi baseline selesai dan sudah disimpan.\n\n"
            "Data ini tersimpan PERMANEN di memori flash ESP32 -- tetap ada "
            "walau device di-restart atau kabel dicabut, sampai kamu hapus "
            "manual (tombol BASE(Z)) atau timpa dengan kalibrasi baru "
            "(tombol KALIBRASI lagi)."
        )
        box.exec_()

    def _handle_session_summary(self, data):
        self.check_session_active = False
        self.check_session_start_time = None
        self.btn_cek1m.setEnabled(True)
        self.btn_cek1m.setText("SESI(K)")
        self._update_health_widget(data.get("dominant", "N/A"), data.get("avg_health", 0.0))
        self._change_page(3)  # 3 = index halaman SUMMARY
    def _update_health_widget(self, dominant_status, avg_health):
        emoji_map = {"normal": "😊", "waspada": "😐", "bahaya": "🛑", "diam": "😴"}
        self.lbl_health_emoji.setText(emoji_map.get(str(dominant_status).strip().lower(), "❓"))
        self.lbl_health_percent.setText(f"{avg_health:.0f}%")
    def _show_session_summary_dialog(self, data):
        """Tampilan kesimpulan versi pertama: popup ringkas. Sengaja bikin
        sesederhana mungkin dulu (bukan panel custom) supaya alur
        tombol->tunggu->kesimpulan ini kebukti jalan end-to-end dulu;
        tampilan bisa dipercantik belakangan kalau alurnya sudah pasti
        benar."""
        dominant = data.get("dominant", "N/A")
        n_normal = data.get("n_normal", 0)
        n_waspada = data.get("n_waspada", 0)
        n_bahaya = data.get("n_bahaya", 0)
        n_diam = data.get("n_diam", 0)
        n_unbalance = data.get("n_unbalance", 0)
        n_misalign = data.get("n_misalign", 0)
        n_bpfo = data.get("n_bpfo", 0)
        n_bpfi = data.get("n_bpfi", 0)
        avg_health = data.get("avg_health", 0.0)
        temp_start = data.get("temp_start", 0.0)
        temp_end = data.get("temp_end", 0.0)
        temp_delta = data.get("temp_delta", 0.0)
        total_samples = data.get("total_samples", 0)

        dominant_lower = str(dominant).strip().lower()
        icon = {"normal": "✅", "waspada": "⚠️", "bahaya": "🛑", "diam": "⏸️"}.get(dominant_lower, "ℹ️")

        pesan = (
            f"{icon} Kondisi dominan selama 1 menit: {dominant}\n\n"
            f"Rincian status ({total_samples} sample):\n"
            f"  Normal={n_normal}  Waspada={n_waspada}  Bahaya={n_bahaya}  Diam={n_diam}\n\n"
            f"Diagnosa yang terdeteksi:\n"
            f"  Unbalance={n_unbalance}  Misalignment={n_misalign}  BPFO={n_bpfo}  BPFI={n_bpfi}\n\n"
            f"Health Score rata-rata: {avg_health:.1f}%\n"
            f"Suhu: {temp_start:.1f}°C -> {temp_end:.1f}°C (delta {temp_delta:+.1f}°C)"
        )

        box = QMessageBox(self)
        box.setWindowTitle("Hasil Sesi Check (1 Menit)")
        box.setText(pesan)
        if dominant_lower == "bahaya":
            box.setIcon(QMessageBox.Critical)
        elif dominant_lower == "waspada":
    ...
    def _evaluate_diagnosis(self, v, temp, device_status=""):
        ...
        status_map = {
            ...
            # FIX: sebelumnya key ini gak ada di dictionary -- padahal
            # firmware kirim status_label persis "Calibrating" (bukan
            # "NotCalibrated") sepanjang 180 detik proses kalibrasi
            # berjalan (lihat main.ino, blok addCalibrationSample()).
            # Tanpa baris ini, status_map.get() gagal cocok, jatuh ke
            # default ("● SYSTEM ONLINE") -- dashboard kelihatan seolah
            # sistem sudah aktif memonitor padahal device-nya masih sibuk
            # ngumpulin data kalibrasi, belum bisa dipercaya hasilnya.
            "calibrating": ("● SEDANG KALIBRASI...", "#888888"),
    ...
    def _update_gui(self):
        ...
        # ===== HITUNG MUNDUR SESI CHECK (tombol SESI(K)) =====
        # Ini CUMA buat tampilan (angka detik di tombol). Yang beneran
        # nutup sesi tetap _handle_session_summary() begitu paket
        # session_summary dari firmware nyampe. Batas aman 15 detik di
        # bawah ini jaga-jaga kalau paketnya gak pernah nyampe (misal
        # serial kecabut di tengah sesi) -- tanpa ini, tombolnya bisa
        # nyangkut "CEK..." selamanya walau device udah gak connect.
        if self.check_session_active and self.check_session_start_time is not None:
            elapsed = time.time() - self.check_session_start_time
            remaining = max(0, self.CHECK_SESSION_DURATION_S - int(elapsed))
            self.btn_cek1m.setText(f"CEK...{remaining}s")
            if elapsed > (self.CHECK_SESSION_DURATION_S + 15):
                self.check_session_active = False
                self.check_session_start_time = None
                self.btn_cek1m.setEnabled(True)
                self.btn_cek1m.setText("SESI(K)")

        # ===== HITUNG MUNDUR + DETEKSI SELESAI KALIBRASI (tombol KALIBRASI) =====
        status_key_now = (self.current_status_device or "").strip().lower()

        # Kalibrasi bisa juga mulai TANPA lewat tombol ini -- firmware
        # otomatis mulai kalibrasi sendiri begitu device baru nyala/reset
        # (lihat setup() di main.ino). Kalau dashboard ketauan statusnya
        # "calibrating" padahal kita gak pernah nge-trigger dari tombol,
        # ikutan lacak dari sini juga -- supaya tombol & hitung mundur
        # tetap nyambung sama kondisi device yang sebenarnya, gak "buta"
        # cuma karena kalibrasinya dipicu otomatis, bukan dari klik user.
        if status_key_now == "calibrating" and not self.calibration_active:
            self.calibration_active = True
            self.calibration_start_time = time.time()
            self.btn_recal.setEnabled(False)

        if self.calibration_active and self.calibration_start_time is not None:
            if status_key_now == "calibrating":
                elapsed = time.time() - self.calibration_start_time
                if elapsed <= self.CALIBRATION_DURATION_S:
                    remaining = max(0, self.CALIBRATION_DURATION_S - int(elapsed))
                    self.btn_recal.setText(f"KALIBRASI...{remaining}s")
                else:
                    # Lewat 180 detik tapi status MASIH "calibrating" --
                    # kemungkinan besar validasi kalibrasi gagal (variansi
                    # terlalu rendah, lihat "[SYSTEM] Kalibrasi GAGAL..." di
                    # InitialBaselineCalibrator.cpp) dan firmware otomatis
                    # mengulang 180 detik lagi dari awal. Dashboard gak bisa
                    # baca pesan teks debug itu (bukan JSON), jadi dikasih
                    # tau apa adanya lewat teks tombol -- lebih baik jujur
                    # dibanding diam-diam nunggu tanpa penjelasan.
                    over = int(elapsed) - self.CALIBRATION_DURATION_S
                    self.btn_recal.setText(f"MENGULANG...({over}s)")
            else:
                # Status BUKAN "calibrating" lagi -> baseline baru resmi
                # sudah tersimpan ke flash. Ini penanda selesai yang
                # sebenarnya, bukan sekadar timer 180 detik habis.
                self.calibration_active = False
                self.calibration_start_time = None
                self.btn_recal.setEnabled(True)
                self.btn_recal.setText("KALIBRASI")
                self._show_calibration_done_dialog()

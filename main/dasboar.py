...
# BARU (27 Agustus 2026, permintaan Ketua Tim): dulu "Nama Mesin" di form
# Tambah Slot itu QLineEdit -- harus DIKETIK. Layar LCD TFT 3.5" di alat gak
# ada keyboard fisik, jadi ngetik nama mesin lewat touchscreen itu gak
# praktis (kalaupun ada on-screen keyboard, tetap lambat & rawan typo).
# Solusinya: jenis mesin sekarang dipilih dari daftar kategori mesin rotasi
# kecil yang sudah dikenal (sesuai BAB 2.1 proposal: pompa air, kipas
# angin, blender, vacuum cleaner, bor listrik, mesin jahit), tinggal
# tap/pilih, gak perlu ngetik apa-apa. Tiap kategori sudah punya ikon
# sendiri, jadi icon mesin juga otomatis ikut sesuai jenisnya (bukan lagi
# ditentukan dari mode kalibrasi seperti sebelumnya).
JENIS_MESIN_OPSI = [
    ("🌀", "Kipas Angin"),
    ("💧", "Pompa Air"),
    ("🥤", "Blender"),
    ...
    ("🛠️", "Bor Listrik"),
    ("🧵", "Mesin Jahit"),
    ("🌬️", "Blower / Exhaust Fan"),
    ("📦", "Motor Konveyor"),
    ("⚙️", "Mesin/Motor Rotasi Lainnya"),
]
...
class AddMachineDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # FIX (27 Agustus 2026): dulu QLineEdit (harus diketik) -- diganti
        # QComboBox kategori jenis mesin, tinggal tap di layar TFT tanpa
        # perlu keyboard. Lihat JENIS_MESIN_OPSI di atas.
        self.cmb_jenis = QComboBox()
        self.cmb_jenis.addItems([f"{icon}  {label}" for icon, label in JENIS_MESIN_OPSI])
    ...
    def get_data(self):
        ...
        # FIX (27 Agustus 2026): name & icon sekarang diambil dari kategori
        # Jenis Mesin yang dipilih (bukan dari teks ketikan / mode kalibrasi
        # lagi) -- lihat JENIS_MESIN_OPSI.
        jenis_icon, jenis_label = JENIS_MESIN_OPSI[self.cmb_jenis.currentIndex()]
        return {
            "name": jenis_label,
            "icon": jenis_icon,
...
class Dashboard(QWidget):
    ...
    def _check_session_countdown_tick(self):
        if self.check_session_time_left > 0:
            ...
            # ("UNBALANCE"/"MISALIGNMENT"/"BEARING_FAULT"/"N/A")
            # ke bahasa awam -- juri/operator biasa gak perlu tau istilah
            # teknis vibration analysis. Kalau statusnya Normal atau diagnosis
            # "N/A"/"NORMAL", tampilkan "Tidak ada" (bukan kosongin field,
            # biar jelas itu memang belum/tidak ada masalah, bukan lupa isi).
            #
            # FIX (27 Agustus 2026, permintaan Ketua Tim): dulu bearing fault
            # dipecah jadi "BEARING_BPFO" (outer race) vs "BEARING_BPFI"
            # (inner race) -- terlalu teknis untuk UMKM. Sekarang firmware
            # cuma kirim satu label gabungan "BEARING_FAULT", dan di sini
            # cukup diterjemahkan jadi "Kerusakan Bearing (Bearing Fault)".
            # Key lama BEARING_BPFO/BEARING_BPFI tetap dipetakan ke teks yang
            # sama, jaga-jaga kalau ada firmware lama/data lama yang masih
            # ngirim label lama itu -- biar dashboard tidak nampilin mentah
            # "BEARING_BPFO" ke operator kalau kejadian.

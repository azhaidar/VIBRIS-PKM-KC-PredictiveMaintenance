...
class Dashboard(QWidget):
    ...
    def _check_session_countdown_tick(self):
        if self.check_session_time_left > 0:
            ...
            self.lbl_rincian_getaran.setText(f"{self.current_v:.2f} mm/s")
    ...
    def _page_raw(self):
        ...
        self.graph_v = pg.PlotWidget(title="Vibration (mm/s)")
        ...
        _tidy_plot(self.graph_v, "Vibration (mm/s)")
    ...
    def _page_summary(self):
        ...
        # FIX (31 Agustus 2026): gauge ini dulu diskalakan buat accelerometer
        # mentah dalam G (max 0.5G, warn 0.18G, danger 0.25G) -- angka itu
        # gak berpatokan ke standar apapun, cuma perkiraan kasar pas
        # dashboard masih baca G langsung. Sekarang rms_v yang masuk ke sini
        # sudah dalam mm/s (RMS velocity, hasil konversi FFT), jadi
        # threshold-nya diganti pakai batas ISO 10816-1 Class I (mesin
        # kecil <15kW, sesuai Tabel 3.2 proposal): Good <0.71 mm/s,
        # Satisfactory 0.71-1.80 mm/s, Unsatisfactory >1.80 mm/s. Max gauge
        # dikasih headroom sampai 5.0 mm/s (di atas batas Unacceptable
        # 4.50 mm/s) biar jarum gak mentok pas kondisi paling parah.
        self.gauge_v = GradientGauge("Vibration", 0.0, 5.0, 0.71, 1.80, "mm/s")
    ...
    def _page_log_detail(self):
        ...
        self.graph_logdet_v = pg.PlotWidget(title="Vibration (mm/s)")
    ...
    def _read_serial_worker(self):
        while True:
            try:
                ...
                # FIX (31 Agustus 2026): firmware (RaspberryPiDataTransmitter.cpp)
                # ganti nama field JSON dari "rms_v" jadi "rms_v_mms" pas satuannya
                # diubah dari G ke mm/s (lihat DriverGetaran.cpp/FFTProcessor.cpp).
                # Baris ini KELEWATAN gak ikut diupdate -- akibatnya
                # data.get("rms_v", 0.0) selalu balikin nilai default 0.0 karena
                # key "rms_v" itu sendiri sudah gak pernah dikirim lagi, silent
                # (gak ada error, gak ada exception, cuma diam-diam selalu 0).
                self.current_v = float(data.get("rms_v_mms", 0.0))
    ...
    def _update_gui(self):
        ...
        if self.current_v is not None:
            ...
            self.lbl_val_v.setText(f"{self.current_v:.2f} mm/s")
            ...
            self.lbl_proc_snapshot.setText(
                f"Live | Vib: {self.current_v:.2f} mm/s | Snd: {self.current_a:.1f} dB | "
            ...
            if self.tick != self.last_processed_tick:
                ...
                if status_key in ("waspada", "bahaya"):
                    ...
                    event_txt = (
                        ...
                        f"Vib {self.current_v:.2f}mm/s, Tmp {self.current_temp:.1f}°C"
                    )

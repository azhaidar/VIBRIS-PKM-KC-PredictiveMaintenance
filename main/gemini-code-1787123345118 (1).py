# ==============================================================================
# VIBRIS INDUSTRIAL HMI - OPERATIONAL ENGINE (UMKM FIELD-READY EDITION)
...
class Dashboard(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("HMI | VIBRIS PIMNAS - UMKM OPERATIONAL")
        self.setWindowFlags(
        ...
        self.machines = [
            ...
            {"name": "Kompresor Produksi", "icon": "🗜️", "bearing_cmd": "B", "rpm_cluster": "High (> 3000 RPM)", "baseline_d2": 11.00, "fw_cluster": "Klaster B"},
            {"name": "Mesin Blender UMKM", "icon": "🥤", "bearing_cmd": "N", "rpm_cluster": "High (> 3000 RPM)", "baseline_d2": 12.50, "fw_cluster": "Klaster B"},
        ]
        ...
        # =========================================================================
        # HEADER FULL FEATURE & DIRECT ACTION (UMKM READY - 1600+ LINES REBUILT)
        # =========================================================================
        header = QHBoxLayout()
        header.setSpacing(1)
        ...
        self.lbl_machine_active = QPushButton("⚙️  - Pilih Mesin UMKM -")
    ...
    def _page_raw(self):
        ...
        lbl_ai_title = QLabel("🤖 UMKM HEALTH & DIAGNOSTICS")
    ...
    def _page_recording(self):
        ...
        self.btn_toggle_rec = QPushButton("MULAI PENCATATAN MESIN")
        ...
        self.lbl_rec_status = QLabel("● Siap Catat Log")
        ...
        lbl_daftar = QLabel("Arsip Riwayat Kerusakan Mesin UMKM (.CSV):")
        ...
        btn_lay = QHBoxLayout()
        self.btn_watch_log = QPushButton("Buka Panel Forensik")
        self.btn_export_excel = QPushButton("Export Excel UMKM")
        self.btn_delete_log = QPushButton("Hapus Berkas")
        ...
        lbl_hint = QLabel("* Klik dua kali untuk investigasi riwayat anomali kerusakan mesin.")
    ...
    def _page_processed(self):
        ...
        lbl_anomali_title = QLabel("Log Kejadian Anomali Operasional:")
        ...
        self.list_anomali.addItem("Tidak ada anomali terdeteksi dalam sesi ini.")
    ...
    def _page_summary(self):
        ...
        self.lbl_diag_desc_summary = QLabel("Menunggu analisis operasional...")
    ...
    def _page_machine_select(self):
        ...
        top_row = QHBoxLayout()
        lbl_title = QLabel("Pilih Mesin UMKM & Klaster Firmware")
        ...
        self.lbl_machine_hint = QLabel("Pilih profil mesin sesuai aset operasional lapangan UMKM Anda.")
    ...
    def _activate_inline_add_machine(self):
        ...
        lbl_nm = QLabel("Nama Mesin UMKM Baru:")
    ...
    def _on_machine_card_clicked(self, idx):
        if self.machine_delete_mode:
            ...
            if self.selected_machine_idx == idx:
                self.selected_machine_idx = -1
                self.lbl_machine_active.setText("⚙️  - Pilih Mesin UMKM -")
    ...
    def _page_log_detail(self):
        ...
        self.lbl_logdet_title = QLabel("FORENSIK KERUSAKAN MESIN: -")
    ...
    def _logdet_render_diagnosis_summary(self):
        ...
        if peak_v > 0.25 or peak_temp > 50.0:
            status, col = "BAHAYA (Potensi Terbakar/Rusak)", COL_BAD
        elif peak_v > 0.18 or peak_temp > 42.0:
            status, col = "WASPADA (Degradasi Awal)", COL_WARN
        else:
    ...
    def _evaluate_diagnosis(self, v, temp, device_status=""):
        ...
        status_map = {
            "diam": ("● MESIN MATI / DIAM", COL_IDLE),
            "bahaya": ("● RISIKO GAGAL / TERBAKAR", COL_BAD),
            "waspada": ("● DEVIASI OPERASIONAL", COL_WARN),
            "normal": ("● ASET AMAN PRODUKSI", COL_OK),
            "warming": ("● WARMING UP MESIN", "#888888"),
            "notcalibrated": ("● KALIBRASI BASELINE", "#888888"),
            "sensorfault": ("● SENSOR FAULT", COL_WARN),
        }
        
        txt, col = status_map.get(status_key, ("● ASET AMAN PRODUKSI", COL_ACCENT))
        if self.packet_loss_flag:
    ...
    def _show_debug_info(self):
        ...
        info = (
            ...
            f"Mesin UMKM Aktif : {selected_machine_name} (Baseline D²: {active_baseline})\n"
        ...
        QMessageBox.information(self, "DEBUG - Info Lapangan UMKM", info)
    ...
    def _update_gui(self):
        ...
        if self.current_v is not None:
            ...
            self.lbl_sys_status.setText("● MENCARI SENSOR ESP32...")
    ...
    def _toggle_recording(self):
        if self.selected_machine_idx < 0:
            QMessageBox.warning(self, "Perhatian", "Pilih Profil Mesin UMKM Terlebih Dahulu!")
            return
        ...
        if not self.recording:
            ...
            if self.csv_file:
                self.csv_file.close()
                self.csv_file = None
            self.btn_toggle_rec.setText("MULAI PENCATATAN MESIN")
    ...
    def _open_log_detail(self, item):
        ...
        self.logdet_play_index = 0
        self.btn_logdet_play_pause.setText("▶ PLAY")
        self.lbl_logdet_title.setText(f"FORENSIK KERUSAKAN MESIN: {filename}")
    ...
    def _export_selected_log_to_excel(self):
        current_item = self.log_list.currentItem()
        if not current_item:
            QMessageBox.information(self, "Pilih Rekaman Dulu", "Pilih arsip rekaman mesin UMKM terlebih dahulu.")
            return

        if openpyxl is None:
            QMessageBox.critical(self, "Library Belum Terinstall", "Fitur export Excel membutuhkan modul openpyxl.")
            return
        ...
        try:
            ...
            if not rows:
                QMessageBox.warning(self, "Berkas Kosong", "Arsip rekaman ini kosong.")
                return
            ...
            QMessageBox.information(
                self, "Export UMKM Berhasil",
                f"Laporan berhasil diexport ke format Excel:\n{xlsx_path}\n\nTotal {len(data_rows)} data terekam."
            )
        except Exception as e:
            QMessageBox.critical(self, "Export Gagal", f"Gagal memproses file Excel:\n{e}")

...
struct DetectionResult {
    ...
    char diagnosis_label[20];   // "UNBALANCE" / "MISALIGNMENT" / "BEARING_FAULT" / "NORMAL" / "N/A"
                                 // FIX (27 Agustus 2026): BEARING_BPFO & BEARING_BPFI digabung jadi
                                 // "BEARING_FAULT" satu label (lihat DiagnosisClassifier.cpp)
    ...
    uint8_t diagnosis_flags;   // BARU: bit0=unbalance, bit1=misalignment, bit2=BPFO, bit3=BPFI
                               // (bit2/bit3 tetap dipisah di flags utk detail internal/log teknis,
                               // cuma diagnosis_label yang digabung jadi "BEARING_FAULT")
};
...
struct CheckSessionSummary {
    ...
    // FIX (27 Agustus 2026): diagnosis_label sekarang cuma keluarin
    // "BEARING_FAULT" (bukan BEARING_BPFO/BEARING_BPFI terpisah), jadi
    // count_diagnosis_bpfo yang menampung hitungannya, count_diagnosis_bpfi
    // selalu 0. Field lama dipertahankan (bukan dihapus) supaya ukuran
    // struct ini tidak berubah -- summary lama yang sudah tersimpan di
    // flash masih bisa dibaca (lihat loadCheckSummaryFromFlash).
};
